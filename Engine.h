...
void EngineRun(RenderTarget renderTarget, Scene& scene) {

	// we need to set up raylib. I just need to open a new window and grab/set
	// an RGB buffer I can write to.

	SetTraceLogLevel(LOG_ERROR); // hopefully puts crashes in logs?
	InitWindow(renderTarget.width, renderTarget.height, "Ivan's Simple Renderer");
